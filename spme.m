...
% Electrolyte polarization coefficient
p.k_conc = (2*p.R*p.T_amb)/(p.alph*p.Faraday) * (1-p.t_plus) * 1;
...
for k = 1:NT
    ...
    % Electrolyte Concentration Term
    kap_n = electrolyteCond(cen_bar(k));
    kap_s = electrolyteCond(ces_bar(k));
    kap_p = electrolyteCond(cep_bar(k));
    
    V_electrolyte(k) = (p.L_n/(2*kap_n) + 2*p.L_s/(2*kap_s) + p.L_p/(2*kap_p))*I(k) ...
                        + p.k_conc*(log(ce0p(k)) - log(ce0n(k)));
    
    % Add 'em up!
    V(k) = V_noVCE(k) + V_electrolyte(k);
